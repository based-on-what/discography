...
class SpotifyDiscographyCreator:
    ...
    def _get_album_tracks_threaded(self, albums: List[Dict]) -> List[str]:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:  # Limit concurrent requests
            # Submit all album track requests
            future_to_album = {
                executor.submit(self._get_album_tracks, album['uri']): album 
                for album in albums
            }
            
            # Process completed requests in order
            for album in albums:  # Maintain album order
                for future in future_to_album:
                    if future_to_album[future] == album:
                        try:
                            track_uris = future.result(timeout=30)  # 30 second timeout
                            all_track_uris.extend(track_uris)
                            logger.info(f"Processed album: {album['name']} ({len(track_uris)} tracks)")
                        except Exception as e:
                            logger.error(f"Failed to process album {album['name']}: {e}")
                        break
